...
// массив, в котором описаны, как выводить символы на экран, последовательность символом как в ascii коде
...
// функция на прорисовку 1 символа
...
void Draw_Simbol(uint16_t x, uint16_t y, uint16_t color, uint16_t phone, uint8_t ascii,  uint8_t size){
	for (int i = 0; i <FONT_Y; i++ )
	{
		...
		{
			if(((simbols[ascii-0x20][i])>>(7-f))&0x01){ // считываем символ, вычитаем 0х20 ( т.к. символы в ascii коде имеют код с 32)
				tft_draw_pixel_4_size(x+f*size,y+i*size,color);
			}
			else{
				tft_draw_pixel_4_size(x+f*size,y+i*size,phone);
